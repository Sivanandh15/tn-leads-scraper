...
Updated: covers all B2B industry types relevant to corporate gifting.
...
CITY_BBOX = {
    ...
    # Other Indian cities (for future expansion)
...
def _query_overpass(tag_key: str, tag_val: str, bbox: tuple, limit: int = 500) -> list[dict]:
    ...
    # No hard limit in the query itself — we cap in Python to stay polite
    overpass_ql = f"""
    [out:json][timeout:60];
    (
...
def _guess_industry(query: str) -> str:
    ...
    if "pharma" in q or "hospital" in q:               return "Pharma / Healthcare"
